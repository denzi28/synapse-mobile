...
const STORAGE_ONBOARDING_SEEN = '@synapse/onboarding_seen_v1';
...
function AppShell() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const seen = await AsyncStorage.getItem(STORAGE_ONBOARDING_SEEN);
  ...
  const tryShowScreenshotIntro = useCallback(() => {
    void (async () => {
      const onboardSeen = await AsyncStorage.getItem(STORAGE_ONBOARDING_SEEN);
  ...
  const onScreenshotChipPress = useCallback(() => {
    void (async () => {
      if (screenshotSyncOn && !isAndroidExpoGo()) {
        const granular = mediaLibraryGranularPhoto();
        const perm = await MediaLibrary.getPermissionsAsync(false, granular);
        if (!perm.granted) {
          await requestScreenshotSync();
          return;
        }
      }
      if (screenshotSyncOn) {
  ...
  /** After onboarding is closed (or skipped), offer screenshot sync once—same check on cold start if onboarding already seen. */
  useEffect(() => {
    if (onboardingOpen) return;
    const introTimer = setTimeout(() => tryShowScreenshotIntro(), 800);
    return () => clearTimeout(introTimer);
  }, [onboardingOpen, tryShowScreenshotIntro]);

  const dismissOnboarding = useCallback(async () => {
    setOnboardingOpen(false);
    try {
      await AsyncStorage.setItem(STORAGE_ONBOARDING_SEEN, 'true');
    } catch {
      /* ignore */
    }
  }, []);
...
Respond strictly in JSON format matching the schema.`;
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      const img = { uri: a.uri, mimeType: a.mimeType ?? 'image/jpeg' };
      setPendingImage(img);
  ...
  const handleSave = async (pickedImage?: { uri: string; mimeType: string } | null) => {
    if (!ensureApiKey()) return;
    const image = pickedImage ?? pendingImage;
    ...
    try {
      ...
      if (image) {
        ...
        const base64Data = await uriToGeminiBase64(image.uri);
        ...
        const response = await ai.models.generateContent({
          model: 'gemini-2.5-flash',
          contents: [{ inlineData: { data: base64Data, mimeType: image.mimeType } }, instruction],
        });
  ...
  return (
    <View style={styles.root}>
      ...
      <Modal
        ...
        animationType="slide"
        transparent
        onRequestClose={() => void dismissOnboarding()}
      >
        <View style={styles.modalRoot}>
          <Pressable style={styles.modalBackdrop} onPress={() => void dismissOnboarding()} />
          <View style={[styles.sheet, { maxHeight: sheetMaxHeight, marginBottom: Math.max(insets.bottom, 16) }]}>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>{t('onboarding.title')}</Text>
              <Pressable
            ...
            <View style={{ paddingHorizontal: 22, paddingBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <Text style={{ color: '#e4e4e7', fontSize: 13, lineHeight: 18, flex: 1 }}>
                  {onboardingPages[Math.min(onboardingStep, onboardingPages.length - 1)]?.title}
                </Text>
              </View>
              <Text style={styles.folderPickExplainer}>
                {onboardingPages[Math.min(onboardingStep, onboardingPages.length - 1)]?.body}
              </Text>

              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, alignItems: 'center' }}>
                <Text style={{ color: '#71717a', fontSize: 12 }}>
                  {onboardingStep + 1}/{onboardingPages.length}
                </Text>
                <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                  <Pressable
                    onPress={() => void dismissOnboarding()}
                    style={({ pressed }) => [styles.onboardingSkipBtn, pressed && styles.pressed]}
                  >
                    <Text style={styles.folderPickSkipText}>{t('onboarding.skip')}</Text>
                  </Pressable>
                  <Pressable
                    onPress={() => {
                      if (onboardingStep >= onboardingPages.length - 1) void dismissOnboarding();
                      else setOnboardingStep((s) => Math.min(s + 1, onboardingPages.length - 1));
                    }}
                    style={({ pressed }) => [styles.onboardingNextBtn, pressed && styles.pressed]}
                  >
